...
PROCESSING_INTERVAL = f"150 seconds"
...
StructField(name="write_ts",dataType=TimestampType())
])
...
def processEachInterval(df:DataFrame,epoch_id):
    ...
    df_ordered = df.orderBy(col("write_ts").desc()).limit(5)
    if df_ordered.count()>0:
        df_ordered.show(truncate=False)
        df_ordered.write.mode("overwrite").parquet(dataSink)
